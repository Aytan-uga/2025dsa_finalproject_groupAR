...
data_clean <- data_raw %>%
  ...
  mutate(
    ...
    Biological.Replication = as.factor(Biological.Replication)  # 🚨 KEEP IT FACTOR
  )
```
 
# Now check bio reps
```{r}
table(data_clean$Biological.Replication)
```

#Ensure categorical variables and Clean data by removing NAs
```{r}
...
```{r}
data_clean %>%
  group_by(Treatment, Material, Biological.Replication) %>%
  summarise(Count = sum(!is.na(Mean_Log_CFU))) %>%
  filter(Count == 0)
