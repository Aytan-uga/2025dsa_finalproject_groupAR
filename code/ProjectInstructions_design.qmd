...
data_clean <- data_raw %>%
  select(
    Date, Treatment, Material, Biological.Replication, Technical.Replication,
    Mean.of.Avg.Population..CFU.ml..Biological.Replication, STDEV
  ) %>%
  rename(
    Mean_CFU_BioRep = Mean.of.Avg.Population..CFU.ml..Biological.Replication
  ) %>%
  filter(
  ...
  mutate(
    ...
    Biological.Replication = as.factor(Biological.Replication)
  ) %>%
  clean_names()
...
mix_mod <- lmer(mean_cfu_bio_rep ~ treatment*material +
