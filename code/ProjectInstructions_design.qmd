...
# 4. Main Effects (if Interaction is Significant)
...
# If interaction is significant (p < 0.05), explore it
# If not, can check main effects separately

## 5. Post hoc comparisons with Tukey adjustment
```{r}
...
For random effects with so few levels (i.e., 3 since that's the number of blocks or biological replicates)
...
# 8. Save summary if needed
```{r}

```

# # 9. Plot Estimated Marginal Means
```{r}

```


# 10. BONUS: Plot Mean Log CFU by Treatment and Material
```{r}
