...
***Hypothesis 1:*** Chemical sanitizing agents will be effective in inactivating *Salmonella* and *Listeria*. Treating food contact surfaces with commercially available chemicals containing chlorine and peracetic acid as a source of reactive oxygen species will decrease the potential pathogens *via* oxidizing essential parts of bacteria.
...
- Interlocking-belt polypropylene
...
```{r}
data_raw <-read.csv("../data/Copy_Listeria.csv")
...
#Check column names
```{r}
data_raw_1 <- clean_names(data_raw) 
data_raw_1
colnames(data_raw_1)
```
...
```{r}
data_clean <- data_raw_1 %>%
  select(
    date, treatment, material, biological_replication, technical_replication,
    mean_of_avg_population_cfu_ml_biological_replication,
    mean_log_reduction_cfu_coupon
  ) %>%
  rename(
    mean_CFU_biorep = mean_of_avg_population_cfu_ml_biological_replication,
    mean_log_reduction = mean_log_reduction_cfu_coupon
  ) %>%
  filter(
    !is.na(treatment) & !is.na(material)
  ) %>%
  mutate(
...
data_clean %>%
  ...
  drop_na(mean_log_reduction)
```
 
# Now check bio reps
```{r}
table(data_clean$biological_replication)
```

# 3. Mixed-Effects Model (Random Effect for Biological Replication)
```{r}
# Changing to sum-to-zero contrast
options(contrasts = c("contr.sum", "contr.poly"))

# Fit the mixed-effects model
mix_mod <- lmer(mean_CFU_biorep ~ treatment + material +
...
# Do Tukey pairwise comparisons
```{r}
pairs(emm_simple, adjust = "tukey")
...
```{r}
leveneTest(Mean.Log.Reduction..CFU.Coupon. ~ Treatment + Material, data = data_clean)
```
